// account.js
//import  {accountsModel,barber,shopmanager,Customer} from '../models/accountsSchema';
...
// Account Model
const  {accountsModel,barber,shopmanager,Customer} = require('../models/accountsSchema');

/* Account API Routes */

// Get all accounts
router.get('/', (req, res) => {
    accountsModel.find().exec()
...
router.get('/:id', (req, res) => {
    accountsModel.findOne({_id: req.params.id}).exec()
        .then(accounts => res.json(accounts))
        .catch(err => res.json(err));
});
...
router.post('/signup', (req, res) => {
    const errorMessage = [];
    const { firstName, lastName, email, phoneNum, password, cpassword } = req.body;
    ...
    if (errorMessage.length > 0) {
        ...
        accountsModel.findOne({email: req.body.email})
            .then(account => {
                if (account != null) {
                    ...
                    const newAccount = new Customer(account);
